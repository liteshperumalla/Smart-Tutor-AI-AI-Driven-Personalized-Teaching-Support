...
def _compute_retrieval_metrics(
    ...
    friendly_topics_found = sorted({topic for topic in expected_topics if topic.lower() in total_topics_found})
...
def _compute_generation_metrics(response_text: str, expected_topics: List[str]) -> Dict[str, Any]:
...
class EvaluationService:
    ...
    def _load_index(self):
        if self._index is None:
            ...
            if storage_context is None:
                raise RuntimeError("Knowledge base is not initialized for evaluation")
    ...
    def run_tests(
        ...
        if difficulties:
            cases = [case for case in cases if case.get("difficulty") in set(difficulties)]
        if limit:
            cases = cases[:limit]
    ...
    def _run_single_case(self, case: Dict[str, Any]) -> Dict[str, Any]:
        ...
        diagnostics = {
            "documents": [
                {
                    "text_excerpt": node.node.get_text()[:400] if hasattr(node.node, "get_text") else "",
        ...
        retrieval_metrics = _compute_retrieval_metrics(diagnostics["documents"], case.get("expected_topics", []))
        generation_metrics = _compute_generation_metrics(response_text, case.get("expected_topics", []))
        ...
        self.evaluator.log_query(
            ...
            metadata={"mode": "evaluation", "case_id": case["id"], "category": case.get("category")},
        )
    ...
    def _analyze_results(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        def avg_metric(lst: List[Dict[str, Any]], key: str) -> float:
            values = [item.get(key) for item in lst if key in item and item.get(key) is not None]
            return round(statistics.mean(values), 3) if values else 0.0

        retrieval_metrics = [r["retrieval_metrics"] for r in results if r.get("retrieval_metrics")]
        generation_metrics = [r["generation_metrics"] for r in results if r.get("generation_metrics")]
        ...
        return {
            ...
            "avg_topic_coverage": round(statistics.mean(coverages), 3) if coverages else 0.0,
            "coverage_above_80": sum(1 for c in coverages if c >= 0.8) / len(coverages) if coverages else 0.0,
            "coverage_above_60": sum(1 for c in coverages if c >= 0.6) / len(coverages) if coverages else 0.0,
            "retrieval_summary": {
            ...
            "generation_summary": {
                "avg_relevance_score": avg_metric(generation_metrics, "relevance_score"),
