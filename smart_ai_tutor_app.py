...
def convert_text_to_pdf(text, output_path):
    ...
    # Add Unicode font (must be TTF and support wide Unicode)
    font_path = "/Users/liteshperumalla/Fonts/ArialUnicode.ttf"  # Change this path if needed
...
def convert_docx_to_pdf(docx_file, output_path):
    doc = DocxDoc(docx_file)
    text = "\n".join([para.text for para in doc.paragraphs])
    convert_text_to_pdf(text, output_path)
    return text

def convert_pptx_to_pdf(pptx_file, output_path):
...
def quiz_generator_sidebar():
    ...
    synthesizer = get_response_synthesizer(response_mode="compact")
    rag_engine = RAGQueryEngine(retriever=retriever, response_synthesizer=synthesizer)

    # Generate quiz
    if not st.session_state.quiz:
        if st.button("Start Quiz"):
            with st.spinner("Generating quiz questions..."):
                st.session_state.quiz = [rag_engine.custom_query("Generate a quiz question") for _ in range(5)]
                st.session_state.correct_answers = [rag_engine.get_correct_answer(q) for q in st.session_state.quiz]
                st.session_state.modules = [rag_engine.get_related_module(q) for q in st.session_state.quiz]
                st.rerun()
    ...
    if st.session_state.quiz:
        ...
        st.progress((q_index + 1) / total_q)  # ✅ Progress bar

        question = st.session_state.quiz[q_index]
        st.write(question)

        st.radio("Your Answer:", ["A", "B", "C", "D"], key=f"answer_{q_index}")
        user_answer = st.session_state.get(f"answer_{q_index}")
        ...
        if st.button("Submit Answer"):
            ...
            correct_answer = st.session_state.correct_answers[q_index]
            st.session_state.user_answers.append(user_answer)

            if user_answer == correct_answer.split()[0]:
                st.success("Correct answer!")
                st.session_state.score += 1
            else:
                st.error(f"Wrong answer. Correct: {correct_answer}")
                st.info(st.session_state.modules[q_index])

            st.session_state.current_q += 1

            if st.session_state.current_q >= total_q:
                st.markdown("## ✅ Quiz Complete")
                st.markdown(f"**Score:** {st.session_state.score} / {total_q}")
                ...
                for i, question in enumerate(st.session_state.quiz):
                    ...
                    if st.session_state.user_answers[i] != st.session_state.correct_answers[i].split()[0]:
                        st.markdown(f"- 📘 Related Module: {st.session_state.modules[i]}")

                # ✅ Save results to file
                save_quiz_results()

                if st.button("Restart Quiz"):
                    for key in ["quiz", "current_q", "score", "user_answers", "correct_answers", "modules"]:
                        st.session_state[key] = [] if key != "current_q" else 0
                    st.rerun()
            else:
                st.rerun()
...
def save_quiz_results():
    ...
    for i, question in enumerate(st.session_state.quiz):
        data["questions"].append({
            ...
            "related_module": st.session_state.modules[i] if st.session_state.user_answers[i] != st.session_state.correct_answers[i].split()[0] else None
        })
...
def about_page():
    st.title("📘 About Smart AI Tutor")

    st.markdown("""
        ### 🎯 Purpose of the App
        **Smart AI Tutor** is an intelligent, interactive platform designed to enhance the way students learn, engage with content, and test their knowledge. Powered by cutting-edge AI and RAG (Retrieval-Augmented Generation) technology, this app supports:

        - 📄 Chatting with uploaded documents (PDF, DOCX, PPTX, TXT)
        - ❓ Generating quizzes from custom content
        - 🧠 Personalized tutoring experiences
        - 🗂 Resource management for effective learning
        - 📅 Scheduling appointments with tutors & Professor.

        Our goal is to make education more accessible, engaging, and personalized for students, educators, and lifelong learners.

        ---

        ### 🛠️ Technology Stack
        - **Frontend:** Streamlit.
        - **Backend:** LlamaIndex, RAGQueryEngine, Python.
        - **File Handling:** PyMuPDF, python-pptx, python-docx.
        - **Chat & NLP:** Ollama, llama_index.
        ...
        **Litesh Perumalla**  
        - 📧 Email: litesh@example.com  
        - 🔗 GitHub: [https://github.com/liteshperumalla](https://github.com/liteshperumalla)
        - 🌐 LinkedIn: [https://www.linkedin.com/in/perumalla-litesh/](https://www.linkedin.com/in/perumalla-litesh/)
        - 🧠 Developer
...
def feedback_and_bug_report():
    ...
    st.markdown("We appreciate your feedback and help in making **Smart AI Tutor** better!")

    tab1, tab2 = st.tabs(["📝 Give Feedback", "🐛 Report a Bug"])
    ...
    with tab1:
        ...
        feedback_text = st.text_area("Your Feedback", height=200, key="feedback_text")
    ...
    with tab2:
        st.subheader("🐞 Found a bug?")
        bug_name = st.text_input("Your Name (optional)", key="bug_name")
        bug_email = st.text_input("Your Email (optional)", key="bug_email")
        bug_title = st.text_input("Bug Title / Short Description", key="bug_title")
        bug_description = st.text_area("Bug Details (steps to reproduce, screenshots, etc.)", height=250, key="bug_description")

        if st.button("Report Bug"):
            if bug_description.strip() and bug_title.strip():
                with open("bug_reports_log.txt", "a") as f:
